...
function initializeServiceWorker() {
  ...
  if ('serviceWorker' in navigator){
    ...
    console.error("Service Worker not supported in browser")
  }
...
async function getRecipes() {
  ...
  // prmise.result: undefined  --- value/ error
  ...
  let promise = new Promise(async function(resolve, reject) {
    ...
    for (let i = 0; i < RECIPE_URLS.length; i++){
      try{
        ...
        tofetch.push(data);
        if (tofetch.length == RECIPE_URLS.length){
            // retrived all data
            saveRecipesToStorage(tofetch);
            resolve(tofetch)
          }
        }catch(error){
          console.error("ERROR fecting file from: ",RECIPE_URLS[i]);
          reject(error);
        }
      }
  });
